		...
		*/
		...
		/*for( int region = svtIdealVolumeFactory.getRegionMin()-1; region < svtIdealVolumeFactory.getRegionMax(); region++ )
			for( int sector = svtIdealVolumeFactory.getSectorMin()[region]-1; sector < svtIdealVolumeFactory.getSectorMax()[region]; sector++ )
			{
				...
				{
					for( int strip = 0; strip < SVTConstants.NSTRIPS; strip+=64 ) // SVTConstants.NSTRIPS
					{
					...
					{
						Geant4Basic cornerBall = new G4Tubs("cornerBall"+i+"_m"+module+"_s"+sector+"_r"+region, 0, cornerBallRadius, cornerBallRadius, 0, 360 ); // cm
						cornerBall.setPosition( layerCorners[i].times(0.1) ); // mm -> cm
